...
using namespace std;
int replaceWithLargerNodesSum(BinaryTreeNode<int> *root, int carry)
{
    if (root == NULL)
    {
        return 0;
    }
    int right = replaceWithLargerNodesSum(root->right, carry);
    int returnValue = root->data + right;
    root->data = root->data + right + carry;
