...
using namespace std;
#include <vector>
void rootToLeafPathSumToK(BinaryTreeNode<int> *root, int k, vector<int> *path)
{
    if (root == NULL)
    {
        return;
    }
    else if ((k - root->data == 0) && (root->left == NULL) && (root->right == NULL))
    {
    ...
    rootToLeafPathSumToK(root->left, k - root->data, path);
    rootToLeafPathSumToK(root->right, k - root->data, path);
    path->pop_back();
}
...
void rootToLeafPathsSumToK(BinaryTreeNode<int> *root, int k)
{
    ...
    rootToLeafPathSumToK(root, k, path);
    delete path;
}
