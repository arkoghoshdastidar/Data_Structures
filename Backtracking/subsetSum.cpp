...
int countSubset(int *arr, int size, int k, vector<int> output)
{
    if (size == 0)
    {
        int sum = 0;
    ...
    int sum1 = countSubset(arr + 1, size - 1, k, output);
    output.push_back(arr[0]);
    int sum2 = countSubset(arr + 1, size - 1, k, output);
    return sum1 + sum2;
}
...
int main()
{
    ...
    while (T--)
    {
        ...
        vector<int> output;
        cout << countSubset(data, N, k, output) << endl;
    }
    return 0;
}