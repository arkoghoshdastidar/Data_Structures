...
{
    priority_queue<int> ticket_priority;
    queue<pair<int, int>> people; // pair.first = priority, pair.second = index
    ...
    while (!people.empty())
    {
        if (people.front().first == ticket_priority.top())
        {
            time++;
            if (people.front().second == k)
            {
                break;
            }
            ticket_priority.pop();
        ...
        pair<int, int> temp = people.front();
        people.pop();
        people.push(temp);
    }
    return time;
