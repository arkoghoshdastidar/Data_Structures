...
#include <cmath>
int getAllWays(int a, int b, int *output)
{
    if (a == 0)
    {
        return 1;
    ...
    for (int i = 1; i <= limit; i++)
    {
        ...
        if (output[i] == -1)
        {
            ...
            temp = getAllWays(a - pow(i, b), b, output);
            ans += temp;
            for (int j = 0; j <= i; j++)
            {
                output[j] = -1;
...
{
    int limit = pow(a, 1 / (float)b);
    int *output = new int[limit + 1];
    for (int i = 0; i <= limit; i++)
    {
        output[i] = -1;
    }
    int answer = getAllWays(a, b, output);
    delete[] output;
    return answer;
}
